...
UIMain.alert.icon = UIMain.alert:CreateTexture(nil , "ARTWORK")
UIMain.alert.icon:SetPoint("LEFT", UIMain.alert, "LEFT", 0,0)
UIMain.alert.icon:SetSize(40, 40)
UIMain.alert.icon2 = UIMain.alert:CreateTexture(nil , "ARTWORK")
UIMain.alert.icon2:SetPoint("RIGHT", UIMain.alert, "RIGHT", 0,0)
UIMain.alert.icon2:SetSize(40, 40)
				...
				if (list4[i] ~= "")then
					if (list2[i] >= tonumber(list4[i])) then
						if finished[i] == false then
							...
							UIMain.alert.icon:SetTexture(list3[i])
							UIMain.alert.icon2:SetTexture(list3[i])
							time = GetTime()
							PlaySound(888 , "SFX")
							finished[i] = true
						end
...
function reset()
	...
	UIMain.addbox:SetShown(false)
	UIMain.confirm:SetShown(false)
	UIMain.settingsb.bg:SetColorTexture(0, 0, 0, 1)
	init()
	getList()
	PopulateLines()
end
